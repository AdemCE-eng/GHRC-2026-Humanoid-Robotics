...
from src.lerobot.utils.logging_utils import AverageMeter, MetricsTracker
...
def train(cfg: TrainPipelineConfig, accelerator: Accelerator | None = None):
    ...
    if (cfg.policy.pretrained_path and not cfg.resume) or not cfg.policy.pretrained_path:
        ...
        processor_kwargs["dataset_stats"] = truncated_stats
    ...
    # Pretrained preprocessor loaded from JSON does not contain the slicer step.
    # Prepend it so the 48-dim state is truncated to 20-dim before feeding the model.
    if cfg.policy.pretrained_path is not None:
    ...
    rabc_weights = None
    if cfg.use_rabc:
        from src.lerobot.utils.rabc import RABCWeights
